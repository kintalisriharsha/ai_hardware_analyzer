# fan_monitor.ps1
# Improved script for gathering fan information
...
# Dynamic fan speed calculation based on CPU load and temperature
if ($isNotebook) {
    # Laptops typically have a single CPU fan
    $fanStatus = "Active"
    
    # More dynamic fan speed calculation based on CPU load and temperature
    if ($cpuLoad -lt 10) {
    ...
    # CPU Fan with more dynamic behavior
    $cpuFanStatus = "Active"
    ...
    # More dynamic chassis fans
    # First chassis fan generally runs faster
    ...
    # Second chassis fan usually runs a bit slower
